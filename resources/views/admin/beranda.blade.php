        ...
        <main class="flex-1 h-full px-8 md:px-16 py-10 overflow-y-auto overflow-x-hidden relative custom-scrollbar">
            ...
            <div class="bg-white p-8 md:p-10 rounded-[3rem] shadow-xl shadow-slate-200/50 border-[8px] border-white hover:border-slate-50 hover:shadow-2xl transition-all duration-500 relative z-10 w-full overflow-x-auto">
                ...
                <div class="flex justify-between items-center mb-8">
                    <h3 class="text-2xl font-inter font-black text-[#272831] tracking-tighter">Daftar Desa</h3>
                </div>

                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b-2 border-slate-100">
                            <th class="py-5 px-4 text-[#929397] font-inter font-black text-[10px] uppercase tracking-widest whitespace-nowrap">No</th>
                            <th class="py-5 px-4 text-[#929397] font-inter font-black text-[10px] uppercase tracking-widest whitespace-nowrap">Nama Desa</th>
                            <th class="py-5 px-4 text-[#929397] font-inter font-black text-[10px] uppercase tracking-widest whitespace-nowrap">Tentang Desa</th>
                            <th class="py-5 px-4 text-[#929397] font-inter font-black text-[10px] uppercase tracking-widest whitespace-nowrap">Foto Desa</th>
                            <th class="py-5 px-4 text-[#929397] font-inter font-black text-[10px] uppercase tracking-widest whitespace-nowrap">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!-- Memulai perulangan data -->
                        @foreach(['Desa Cimulang', 'Desa Cipularang', 'Desa Sukamaju'] as $index => $namaDesa)
                            
                        <!-- Baris tabel -->
                        <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                            
                            <!-- Nomor Urut -->
                            <td class="py-5 px-4 text-sm font-inter font-bold text-[#272831]">{{ $index + 1 }}</td>
                            
                            <!-- Nama Desa -->
                            <td class="py-5 px-4 text-sm font-inter font-bold text-[#272831]">{{ $namaDesa }}</td>
                            
                            <!-- Deskripsi -->
                            <td class="py-5 px-4 text-sm font-inter font-medium text-[#929397] max-w-xs truncate">
                                Ini adalah data untuk {{ $namaDesa }}...
                            </td>
                            
                            <!-- Placeholder Foto -->
                            <td class="py-5 px-4">
                                <div class="w-16 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-[10px] font-bold text-slate-400 group-hover:bg-slate-200 transition-colors">
                                    IMG
                                </div>
                            </td>
